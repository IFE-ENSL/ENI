...
{
    public class DragRigibody2D : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            //Debug.DrawLine(mainCamera.ScreenToWorldPoint(Input.mousePosition), Vector2.zero,Color.cyan,5f);
            ...
            if (hit.transform.gameObject.tag != "Moving" || hit.rigidbody.isKinematic == true) {
                return;
            }
            if (hit.collider != null &&  hit.rigidbody.isKinematic == false) {
                if (!springJoint)
                {
                    ...
                    //go.transform.parent = hit.collider.gameObject.transform;
